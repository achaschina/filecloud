...
export class DirTreeViewComponent implements OnInit {
  ...
  ngOnInit() {
    this.apiService.getResource('/', this.currentUser, 'fileasc')
      .subscribe((data: any) => {
        this.resources = { ... data };
  ...
  updateData(path: string) {
    ...
    this.apiService.getResource(path, this.currentUser, 'fileasc')
      .subscribe((data: any) => {
        this.resources = { ... data };
  ...
  // Обрезать имя папки
  getName(item: any) {
    if (item.toString() === 'Мой диск:/') { return 'Мой диск'; }
